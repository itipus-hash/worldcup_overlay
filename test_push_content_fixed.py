...
# 模拟 _format_match_push 方法（修复后）
def format_match_push(m, kind):
    """模拟 _format_match_push 方法（修复后版本）"""
    stage_map = {
        "1h": "上半场",
        "2h": "下半场",
        "ht": "中场休息",
                ...
                if summary:
                    ...
                    if all_goals:
                        ...
                        for g in all_goals:
                            g_team = f"（{g['team_cn']}）" if g.get('team_cn') else ""
                            lines.append(
                                f"  {g['minute']} {g['scorer']}{g_team} {g['method']}  {g['score']}"
                            )
            except Exception as e:
                lines.append(f"（无法获取进球明细：{e}）")
        content = "<br>".join(lines)
...
for i, m in enumerate(matches):
    ...
    # 生成 "end" 类型推送内容（比赛结束）
    print("生成的【比赛结束】推送内容（修复后，包含进球明细）：")
